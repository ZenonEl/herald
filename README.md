...
- `send_update` — короткий управленческий апдейт из отдельных полей;
...
- `brief` — вопрос или тема и прямой короткий ответ, без простыни;
...
GNU Affero General Public License v3.0 or later (`AGPL-3.0-or-later`). Если
изменённая версия Herald работает как сетевой сервис, её пользователям должен
быть доступен соответствующий исходный код. Лицензия распространяется на Herald,
но не присваивает Herald права на сообщения, файлы и другие пользовательские
материалы, которые проходят через него.

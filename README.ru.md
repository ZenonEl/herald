...
чистый текст клиенту и подпись ответом на него. [Шаблоны и настройки
пакетов](docs/batches.md).
...
После установки открой новую сессию. Плагин содержит три skill:
...
`send_file` отправляет одно вложение. `send_files` принимает 1–100 файлов:

- `album` — 2–100 файлов; Herald сам делит их на связанные Telegram-группы по
  десять и ставит подпись только на первую;
- `separate` — 1–100 отдельных сообщений, связанных с первым, без повторения
  общей подписи.

Режим `auto` отправляет небольшие JPEG/PNG/WebP как фотографии, остальное — как
документы. Смешанный альбом нужно отправлять как `document`. Подпись ограничена
1024 отображаемыми символами, файл — настроенным лимитом до 50 МБ.

Если нужны несколько текстовых частей, файлы вместе с отдельной подписью или
единый reply на входящее сообщение, используется `send_batch`. Все части одного
batch идут в один настроенный маршрут; `role` и `tags` — метаданные, а не ACL и
не способ скрытно отправить часть другому адресату. При явном отказе Telegram от
native reply Herald использует проверенный цитатный fallback. Сетевая
неопределённость не повторяется автоматически.
...
Общие skill лежат в `skills/herald-draft`, `skills/herald-send` и
`skills/herald-watch`. При ручной установке обе системы могут использовать один
checkout через символические ссылки для каждого из этих каталогов, но
marketplace-установка предпочтительнее и не требует ссылок. Не смешивай оба
способа.

Пример для одного skill:
...
Code и `$herald-send` в Codex; два остальных skill подключаются аналогично.
...
## Watch: связь с открытыми AI-сессиями
...
- неадресованные сообщения не попадают ни одной сессии; явно посмотреть их
  может только duty того же разрешённого источника;
...
Дежурство автоматически освобождает профили после 15 минут без сигнала от
монитора, опроса или активности ИИ. Незакрытые сообщения возвращаются в очередь:
вчерашняя сессия не сможет навсегда заблокировать новый `watch_start`.
...
завершилась ошибкой. Их можно изменить или отключить в профиле.

Watch принимает адресованные документы, изображения, голосовые и аудиофайлы.
ИИ получает тип, MIME, размер и локальный путь либо безопасную причину, почему
файл не скачан. Голосовое без caption можно отправить ответом на предыдущее
сообщение с `#тегом`. Herald не зависит от Whisper и сам не загружает аудио во
внешний сервис: локальный или API-транскрибатор подключается отдельно.
Ответ с несколькими сообщениями или файлами отправляется через
`watch_reply_batch` по фиксированному маршруту профиля.

Неадресованные сообщения и их вложения доступны только duty того же разрешённого
источника и по умолчанию удаляются через семь дней
(`watch.unaddressed_ttl_days`). Адресованные pending/claimed-доставки эта очистка
не затрагивает.
